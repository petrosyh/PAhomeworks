               ...
               begin
                 let f =
                   begin
                     fun m' ->
                     begin
                       let _ = print_string("******* m' in point *******") in
                       let _ = print_newline() in
                       let _ = pp_memory m' varlist in 
                       let truemem = assume (m', e, nth) in
                       let _ = print_string("******* truemem in point *******") in
                       let _ = print_newline() in
                 ...
                 let _ = print_string("******* widenmem in point *******") in
                 let _ = print_newline() in
                 let _ = pp_memory widen_mem varlist in 
                 let narrow_mem = (fix_with_narrowing f botm widen_mem varlist) in
                 let _ = print_string("******* narrowmem in point *******") in
                 let _ = print_newline() in
                 let _ = pp_memory narrow_mem varlist in 
                 let v = assumeNot (narrow_mem, e, nth) in
                 let _ = print_string("******* resmem in point *******") in
                 let _ = print_newline() in
                 let _ = pp_memory v varlist in
                 v
               end
          end
       | ASSIGN(id, e) ->
